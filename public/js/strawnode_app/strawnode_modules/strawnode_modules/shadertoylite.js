...
function ShaderToyLite(canvasId, forceIndependant) {
    ...
    // outside time == total elapsed time (even if faked while halted)
    // prevDrawTime - firstDrawTime  ===> 3.5 sec - 1.5 (started time)
    this.time = () => {
    ...
    var adapt = (reset) => {
        var now = Date.now();
        var elapsed = !!reset ? 0 : prevDrawTime - firstDrawTime;
        firstDrawTime = now - elapsed;
        prevDrawTime = now;
    }

    // resetting will set both starttime and prevTime to now ;
    // and sets back iFrame before drawing
    this.reset = () => {
        adapt(true) ;
        iFrame = 0;
        draw() ;
    ...
    var anim ;
    var hasBetween = undefined !== window['BetweenJS'] && !!! forceIndependant ;
    var iii = 0 ;
    var thres = 2 ;
    if(hasBetween){
        anim = new BJS.$.Animation(undefined, function(n){
            var loctime = (Date.now() - prevDrawTime) * .01 ; 
    ...
    var draw = () => {
        ...
        // set prev time of last draw for future
        prevDrawTime = now ;
...
if (typeof module !== "undefined" && module.exports) module.exports = ShaderToyLite;
