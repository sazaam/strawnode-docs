...
(function (name, definition) {
	...
	return Pkg.write('org.libspark.straw', function () {
		 ...
		 *
		 *  Uses native `Proxy` when available (dynamic property tracking, no enumeration
		 *  at wrap time). Falls back to static closure enumeration + Type.define in older
		 *  browsers. All API contracts are identical in both paths.
		 *  @static */
		var Proxy = Type.define(function () {
			var ns = {}, __global__ = window, returnValue = function (val, name) { return val },
				 ...
				 *  Used by both the native Proxy handler (for the `original` map) and
				 *  the static fallback.
			...
			var HAS_NATIVE_PROXY = typeof window.Proxy !== 'undefined';
			var NATIVE_PROXY = HAS_NATIVE_PROXY ? window.Proxy : null;

			/** Build a native Proxy handler for a target + override pair.
			 *  Every property access creates a typed closure on the fly — no
			 *  pre‑enumeration needed, so properties added after wrap time are
			 *  automatically visible. */
			var makeNativeHandler = function (tar, over) {
				return {
					get: function (target, prop, receiver) {
						if (prop === '__proxy__') return receiver;
						if (prop === 'base') return tar.constructor;
						if (prop === 'factory') return tar;
						if (prop === 'constructor') return tar.constructor;
						if (prop === 'original') return over.original;
						...
						switch (type) {
							case 'null': case 'undefined': case 'number': case 'string': case 'boolean':
								return function () { return arguments.length > 0 ? (tar[prop] = arguments[0]) : tar[prop]; };
							case 'object':
								return function (o, o2) {
									if (!!o) {
										var tt = typeof o, ob = tar[prop];
										if (tt === 'string' || tt === 'number') return (o2 === undefined) ? ob[o] : (ob[o] = o2);
										for (var s in o) ob[s] = o[s];
										return tar[prop];
									}
									return tar[prop];
								};
							case 'function':
								return function () { return tar[prop].apply(tar, arguments); };
							default:
								return val;
						}
					},
					set: function (target, prop, value) {
						if (prop in over) over[prop] = value;
						tar[prop] = value;
						return true;
					},
					has: function (target, prop) {
						return prop in tar || prop in over;
					}
				};
			};

			/** Populate `over.original` with getPropertyClosure for every property
			 *  on `tar` that also exists in `over`. Shared by both paths. */
			var populateOriginal = function (tar, over) {
				for (var s in tar) {
					if (s === 'constructor' || s === 'toString' || s === '__proxy__') continue;
					if (s in over) {
						over.original[s] = getPropertyClosure(tar[s], s, tar);
					}
				}
			};
			...
			return {
				...
				constructor: Proxy = function Proxy(target, override, toClass) {
					...
					var cl = target.constructor, withoutnew = (this === __global__), tobecached = false, ret;
					var name_r = /function([^\(]+)/;
					var getctorname = function (cl) { return (cl = cl.match(name_r)) ? cl[1].replace(' ', '') : '' };
				...
				// cache hit — skip when an override is provided (baked into the
				// cached closure) or when toClass is true (different constructors
				// may share the same class name, causing prototype chain collisions).
				if (!!ns[cl] && tobecached === true && !override && !toClass) {
					ret = ns[cl];
					return (toClass) ? ret : new ret(target);
				}

					// ── common setup ──
					var tar = target, over = override || {};
					over.original = {};

					if (HAS_NATIVE_PROXY) {
						// ── native Proxy path ──
						// no protoinit needed — the handler intercepts dynamically
						populateOriginal(tar, over);
						var handler = makeNativeHandler(tar, over);
						var proxy = new NATIVE_PROXY(tar, handler);
						tar['__proxy__'] = proxy;
						...
						var store = function (r, ns, cl) {
							if (tobecached === true) {
								var Fact = function (t) {
									var nt = t || tar;
									var nh = makeNativeHandler(nt, over);
									var wrapper = Object.create(Fact.prototype);
									var np = new NATIVE_PROXY(wrapper, nh);
									nt['__proxy__'] = np;
									return np;
								};
								// Link Fact.prototype to the target constructor's prototype
								// so both proxy instanceof Fact and
								// proxy instanceof OriginalConstructor work.
								var baseProto = tar && (tar.constructor ? tar.constructor.prototype : Object.getPrototypeOf(tar));
								if (baseProto) {
									Object.setPrototypeOf(Fact.prototype, baseProto);
								}
								ns[cl] = Fact;
								return (toClass) ? Fact : Fact(tar);
							}
							return r;
						};

						return store(proxy, ns, cl);
					}

					// ── fallback: static closure path ──
					...
					if (tar.constructor && tar.constructor !== Object) {
						over.inherits = over.inherits || tar.constructor;
					}
		...
		/* ─── DOMNodeProxy — event-driven DOM element proxy ─── */

		/**
		 *  DOMNodeProxy — event-driven DOM element proxy.
		 *
		 *  Wraps a DOM Node in a native Proxy that exposes DOM properties and
		 *  attributes as getter/setter closures, DOM events via on/off/once/trigger,
		 *  and CSS animation by delegating to BetweenJS (via BJS.create).
		 *
		 *  ─── Handler dispatch order ───
		 *  When a property is accessed, the handler checks in this order:
		 *    1. Event methods  — on, off, once, trigger  (with @-prefix support)
		 *    2. Traversal      — children, parent
		 *    3. Content        — html, text, val
		 *    4. Class helpers  — addClass, removeClass, toggleClass, hasClass
		 *    5. DOM manip      — append, remove, attr, css
		 *    6. Animation      — animate (compound animation with BetweenJS)
		 *    7. CSS props      — auto-detected from document.body.style (animated if `time` provided)
		 *    8. DOM property   — direct node property, e.g. className, id, disabled
		 *    9. Attribute      — getAttribute/setAttribute fallback
		 *
		 *  ─── Event system and @-prefix ───
		 *  on/off/once/trigger all support the @-prefix shorthand that mirrors
		 *  Express.attachHandler. Maps shorthand names to real event types:
		 ...
		 *      @*          → strips @, passes through
		 *
		 *  These expansions match the Express step lifecycle events, so
		 *  proxy.on('@open', fn) and Express.attachHandler(true, '@open', fn, res)
		 *  produce equivalent bindings through the same Global event relay chain.
		 *  The resolveEvent() helper at the module level keeps this mapping
		 *  self-contained — no dependency on Express being loaded.
		 *
		 *  ─── Relay chain integration ───
		 *  Constructor calls Global.generateDomProxyAlongTarget(node) which
		 *  creates an EventDispatcher chain wired into the Global dispatcher
		 *  tree (_dispatchers / _proxies). When a proxied element lives inside
		 *  an Express Step's DOM subtree, step lifecycle events propagate
		 *  through the relay to the proxy's listeners automatically.
		 *
		 *  ─── Two-way sync ───
		 *  A MutationObserver in the constructor observes attribute changes,
		 *  child list mutations, and character data. On any DOM mutation it
		 *  fires a 'change' event on the dispatcher. This allows declarative
		 *  directives (data-text, data-bind, etc.) to react to external DOM
		 *  changes.
		 *
		 *  ─── Animation model ───
		 *  CSS property setters accept an optional config object. When
		 *  `config.time` is present, the setter delegates to DOMNodeProxy.tween()
		 *  which calls BetweenJS.create() with the options:
		 *      { target, to: { prop: value }, time, ease, delay, ... }
		 *  The `time` field matches BetweenJS's own options['time'] signature
		 *  (not `duration`). When the tween completes, a 'change' event fires.
		 *
		 *  ─── Declarative hookup ───
		 *  hookup(root) scans the subtree for HTML directives:
		 *      data-text="key"    — one-way: scope.key → textContent
		 *      data-bind="key"    — two-way: textContent + input → scope.key
		 *      data-click="expr"  — evaluates JS expression in scope (with(p))
		 *      data-show="key"    — display toggle when scope.key is truthy
		 *      data-hide="key"    — display toggle when scope.key is falsy
		 *      data-scope         — marks the root scope element
		 *  All directives listen on proxy's 'change' event and react when
		 *  e.property matches the bound key.
		 *
		 *  ─── Usage ───
		 *      var el = DOMNodeProxy.find('#app');
		 *      el.className('active');
		 *      el.backgroundColor('#e94560', { time: 800, ease: 'easeOutQuad' });
		 *      el.on('click', function () { el.background('#00f'); });
		 *      el.html('<p>Hello</p>');
		 *
		 *  Declarative hookup:
		 *      DOMNodeProxy.hookup('#app');
		 *
		 *  @alias prox
		 *  @type {Object}
		 */
		var DOMNodeProxy = Type.define(function () {
			var cache = typeof WeakMap !== 'undefined' ? new WeakMap() : null;
			var cssProps = {};
			if (typeof document !== 'undefined' && document.body)
				for (var s in document.body.style) cssProps[s] = true;

			/** Resolve an event type to one or more real event names.
			 *  Unprefixed names pass through unchanged. @-prefixed names use
			 *  the shorthand mapping that mirrors Express.attachHandler:
			 *      @focus      →  focusIn + focusOut
			 *      @open       →  step_opening
			 *      @close      →  step_closing
			 *      @toggle     →  step_opening + step_closing
			 *      @toggleIn   →  step_opening
			 *  @param   {string} type  Event type, optionally @-prefixed
			 *  @returns {string[]}     One or more real event types */
			var resolveEvent = function (type) {
				if (type.charAt(0) !== '@') return [type];
				var raw = type.slice(1);
				switch (raw) {
					case 'focus': return ['focusIn', 'focusOut'];
					case 'toggle': return ['step_opening', 'step_closing'];
					case 'open': case 'toggleIn': return ['step_opening'];
					case 'close': case 'toggleOut': return ['step_closing'];
					default: return [raw];
				}
			};

			var makeHandler = function (node, disp) {
				// Fire a 'change' event with the property name so declarative
				// hookup directives (data-text, data-bind, data-show, data-hide)
				// can check e.property === key and react selectively.
				var fireChange = function (prop) {
					var list = disp._listeners && disp._listeners['change'];
					if (list) list.slice().forEach(function (fn) { fn({ type: 'change', property: prop }); });
				};
				return {
					get: function (target, prop, receiver) {
						// ── Event methods ──────────────────────
						// on(type, fn) / off(type, fn) / once(type, fn) / trigger(type)
						// All support @-prefixed event types via resolveEvent().
						// Each resolved event is bound/unbound on the proxy's
						// EventDispatcher, which is wired into the Global relay
						// chain via generateDomProxyAlongTarget. Callbacks are
						// received via EventDispatcher.bind/unbind which delegates
						// to Global.bind and eventually addEventListener on the DOM
						// node (for native events) or Global.fire (for custom events
						// like step_opening that propagate through _dispatchers).
						if (prop === 'on')
							return function (t, fn) {
								resolveEvent(t).forEach(function (e) { disp.bind(e, fn); });
								return receiver;
							};
						if (prop === 'off')
							return function (t, fn) {
								resolveEvent(t).forEach(function (e) { disp.unbind(e, fn); });
								return receiver;
							};
						if (prop === 'once')
							return function (t, fn) {
								resolveEvent(t).forEach(function (e) {
									var w = function (ev) { fn(ev); disp.unbind(e, w); };
									disp.bind(e, w);
								});
								return receiver;
							};
						if (prop === 'trigger')
							return function (t) {
								resolveEvent(t).forEach(function (e) { disp.trigger(e); });
								return receiver;
							};

						// ── Traversal ──────────────────────────
						// children — returns wrapped proxies for all child elements.
						// parent   — returns wrapped proxy for parent node or null.
						if (prop === 'children')
							return Array.from(node.children).map(wrap);
						if (prop === 'parent')
							return node.parentNode ? wrap(node.parentNode) : null;

						// ── Content ────────────────────────────
						// html(v) — get/set innerHTML. No-arg call returns current value.
						// text(v) — get/set textContent. No-arg call returns current value.
						// val(v)  — get/set .value (for input elements). No-arg returns value.
						// All setters fire a 'change' event on the dispatcher and return
						// the proxy receiver for chaining.
						if (prop === 'html')
							return function (v) { return arguments.length
								? (node.innerHTML = v, receiver) : node.innerHTML; };
						if (prop === 'text')
							return function (v) { return arguments.length
								? (node.textContent = v, receiver) : node.textContent; };
						if (prop === 'val')
							return function (v) { return arguments.length
								? (node.value = v, receiver) : node.value; };

						// ── Class helpers ──────────────────────
						// addClass, removeClass, toggleClass, hasClass.
						// Simple wrappers around DOMTokenList.classList.
						// Mutators return the proxy receiver for chaining.
						if (prop === 'addClass')
							return function (c) { node.classList.add(c); return receiver; };
						if (prop === 'removeClass')
							return function (c) { node.classList.remove(c); return receiver; };
						if (prop === 'toggleClass')
							return function (c) { node.classList.toggle(c); return receiver; };
						if (prop === 'hasClass')
							return function (c) { return node.classList.contains(c); };

						// ── DOM manipulation ───────────────────
						// append(child) — appends a DOM node (or unwrapped proxy).
						// remove()      — removes this node from its parent.
						// attr(n, v)    — get/set attribute. One arg = read, two = write.
						// css(n, v)     — get/set inline style. One arg = computed, two = write.
						// All setters fire 'change' event and return receiver.
						if (prop === 'append')
							return function (child) {
								var el = (child && child.__target__) ? child.__target__ : child;
								node.appendChild(el);
								return receiver;
							};
						if (prop === 'remove')
						...
						if (prop === 'attr')
							return function (name, v) {
								if (arguments.length === 1) return node.getAttribute(name);
								node.setAttribute(name, v);
								fireChange(name);
								return receiver;
							};

						// ── CSS composite (name, val, cfg) ─────
						// css(name)       — read computed style for `name`
						// css(name, val)  — set inline style `name` to `val`, fire change
						// css(name,val,cfg) — animated set: delegates to tween() when
						//                     cfg.time is present. cfg matches betweenJS:
						//     { time: 800, ease: 'easeOutQuad', delay: 100, ... }
						if (prop === 'css')
							return function (name, v, cfg) {
								if (arguments.length === 1) return getComputedStyle(node)[name];
								if (cfg && typeof cfg === 'object' && 'time' in cfg) {
									return DOMNodeProxy.tween(node, name, v, cfg, receiver, disp);
								}
								node.style[name] = v;
								fireChange(name);
								return receiver;
						...
						// ── Multi-prop animation ───────────────
						// animate(props, cfg) — animate several CSS properties at once.
						//   props : { cssProperty: endValue, ... }
						//   cfg   : { time, ease, delay, onStart, onUpdate, onComplete }
						// Single BetweenJS.create() call animates all props in parallel.
						// Fires 'change' on completion. No-op if BetweenJS unavailable
						// (falls through to direct style assignment).
						if (prop === 'animate')
							return function (props, cfg) {
								if (typeof BetweenJS === 'undefined' || !BetweenJS.create) {
									for (var s in props) node.style[s] = props[s];
									return receiver;
								}
								var opts = { target: node, to: props, time: 400, ease: 'easeOutQuad' };
								if (cfg) {
									if (cfg.time !== undefined) opts.time = cfg.time;
									if (cfg.ease) opts.ease = cfg.ease;
									if (cfg.delay) opts.delay = cfg.delay;
									['onStart', 'onUpdate', 'onComplete'].forEach(function (k) {
										if (cfg[k]) opts[k] = cfg[k];
									});
								}
								var t = BetweenJS.create(opts);
								t.bind('complete', function () { disp.fire('change'); });
								t.start();
								return receiver;
							};

						// ── CSS properties (auto-detected) ────
						// Any property found in document.body.style is treated as a CSS
						// property setter. No-arg call returns current computed value.
						// If second arg is an object with `time`, delegates to tween()
						// for animated transitions. Otherwise sets inline style directly.
						// Examples:
						//   el.backgroundColor('#e94560')
						//   el.backgroundColor('#e94560', { time: 800, ease: 'easeOutQuad' })
						if (cssProps[prop]) {
							return function (v, cfg) {
								if (!arguments.length) return getComputedStyle(node)[prop];
								if (cfg && typeof cfg === 'object' && 'time' in cfg) {
									return DOMNodeProxy.tween(node, prop, v, cfg, receiver, disp);
								}
								node.style[prop] = v;
								fireChange(prop);
								return receiver;
						...
						// ── Direct DOM properties (className, id, etc.) ──
						// If prop is a direct property of the DOM node (e.g. className,
						// id, disabled, checked), wraps it as a getter/setter closure.
						// No-arg → read, one arg → write + fire('change') + receiver.
						if (prop in node) {
							return function (v) {
								if (!arguments.length) return node[prop];
								node[prop] = v;
								fireChange(prop);
								return receiver;
							};
						...
						// ── Target properties (proxy internals) ─
						// Properties on the proxy target itself (__target__,
						// __dispatcher__, __observer__, constructor) must
						// be accessible through the handler so callers can
						// reach the underlying dispatcher or DOM node.
						// Return the raw value (not a closure) so that
						// prox.unwrap(p) and p.__dispatcher__ work directly.
						if (prop in target) {
							return target[prop];
						}

						// ── Attribute fallback ─────────────────
						// If prop is not an event method, traversal property, content
						// helper, class helper, DOM manipulation, CSS property,
						// direct DOM property, or target property, fall back to
						// getAttribute/setAttribute. The getter auto-parses numeric
						// attribute values so that `count() + 1` works in data-click
						// expressions (avoids string concatenation).
						// This allows arbitrary data-* attributes and custom HTML
						// attributes to be read/written through the proxy transparently.
						return function (v) {
							if (!arguments.length) {
								var raw = node.getAttribute(prop);
								if (raw === null || raw === undefined) return raw;
								var num = Number(raw);
								return !isNaN(num) && raw !== '' ? num : raw;
							}
							node.setAttribute(prop, v);
							fireChange(prop);
							return receiver;
						};
					},
					...
					set: function (target, prop, value) {
						// Direct property assignment on the proxy sets the DOM node
						// property directly if it exists, otherwise sets an attribute.
						// This allows `el.disabled = true` in addition to the
						// getter/setter closure pattern `el.disabled(true)`.
					...
					has: function (target, prop) {
						// Always return true so the `with(p)` pattern used in
						// data-click expressions works correctly. The `with`
						// statement checks HasProperty before Get — if we return
						// false, the identifier falls through to the enclosing
						// scope instead of resolving through the proxy.
						return true;
					}
				};
			};

			/** Wrap a DOM element in a DOMNodeProxy.
			 *  Uses WeakMap cache to return the same proxy for the same node.
			 *  Falls back to __domproxy__ property on the node if WeakMap unavailable.
			 *  @param   {Node} node  A DOM element (nodeType === 1)
			 *  @returns {Object}     Wrapped proxy, or the node itself if not an element */
			var wrap = function (node) {
				if (!node || node.nodeType !== 1) return node;
				var p = cache ? cache.get(node) : node.__domproxy__;
				if (p) return p;
				p = new DOMNodeProxy(node);
				(cache ? cache.set(node, p) : (node.__domproxy__ = p));
				return p;
			};

			/** Find a DOM element and wrap it in a DOMNodeProxy.
			 *  Accepts a CSS selector string or an existing DOM node.
			 *  @param   {string|Node} selector  CSS selector or DOM element
			 *  @returns {Object|null}           Wrapped proxy, null if not found */
			var find = function (selector) {
				var node = typeof selector === 'string'
					? document.querySelector(selector) : selector;
				return node && node.nodeType === 1 ? wrap(node) : node;
			};

			/** Scan a DOM subtree for declarative directives and wire them to a scope proxy.
			 *  Supports:
			 *      data-text="key"    — one-way binding: scope.key → textContent
			 *      data-bind="key"    — two-way binding: textContent + input → scope.key
			 *      data-click="expr"  — evaluate expression in scope (with(p) { expr })
			 *      data-show="key"    — display:'' when scope.key truthy, else display:none
			 *      data-hide="key"    — display:none when scope.key truthy, else display:''
			 *      data-scope         — marks the root scope element
			 *
			 *  All directives react to scope 'change' events on the proxy dispatcher.
			 *  @param   {string|Node} root  CSS selector or DOM element to scan
			 *  @returns {Object|null}       Wrapped scope proxy, null if not found */
			var hookup = function (root) {
				var el = typeof root === 'string' ? document.querySelector(root) : root;
				if (!el) return null;
				var scope = el.closest ? el.closest('[data-scope]') : null;
				if (!scope) scope = el;
				var proxy = wrap(scope);
				// Scan within the scope element for directives (not within `el`,
				// which may be a descendant or sibling of the scope).
				if (!scope.querySelectorAll) return proxy;

				scope.querySelectorAll('[data-text]').forEach(function (child) {
					var key = child.getAttribute('data-text');
					child.textContent = proxy[key] ? proxy[key]() : '';
					proxy.on('change', function (e) {
						if (e.property === key) child.textContent = proxy[key] ? proxy[key]() : '';
					});
				});

				scope.querySelectorAll('[data-bind]').forEach(function (child) {
					var key = child.getAttribute('data-bind');
					child.textContent = proxy[key] ? proxy[key]() : '';
					proxy.on('change', function (e) {
						if (e.property === key) child.textContent = proxy[key] ? proxy[key]() : '';
					});
					if (/^INPUT|TEXTAREA|SELECT$/.test(child.tagName)) {
						child.addEventListener('input', function () {
							if (proxy[key]) proxy[key](child.value);
						});
					}
				});

				scope.querySelectorAll('[data-click]').forEach(function (child) {
					var expr = child.getAttribute('data-click');
					child.addEventListener('click', function () {
						(new Function('p', 'with(p) { ' + expr + ' }'))(proxy);
					});
				});

				['show', 'hide'].forEach(function (dir) {
					scope.querySelectorAll('[data-' + dir + ']').forEach(function (child) {
						var key = child.getAttribute('data-' + dir);
						var show = dir === 'show';
						var init = proxy[key] ? proxy[key]() : false;
						child.style.display = !!init === show ? '' : 'none';
						proxy.on('change', function (e) {
							if (e.property === key) {
								var v = proxy[key] ? proxy[key]() : false;
								child.style.display = !!v === show ? '' : 'none';
							}
						});
					});
				});
			...
			return {
				pkg: 'proxies::DOMNodeProxy',
				domain: Type.appdomain,
				statics: {
					/** Find and wrap a DOM element. @see DOMNodeProxy.find */
					find: find,
					/** Wrap a DOM element in a proxy. @see DOMNodeProxy.wrap */
					wrap: wrap,
					/** Unwrap a DOMNodeProxy to get the underlying DOM node.
					 *  @param {Object} p  A DOMNodeProxy or any object
					 *  @returns {Node}   The DOM node if p is a proxy, otherwise p */
					unwrap: function (p) { return p && p.__target__ ? p.__target__ : p; },
					/** Scan a DOM subtree for declarative directives and wire to scope.
					 *  @see DOMNodeProxy.hookup */
					hookup: hookup,
					/** Animate a CSS property on a node using BetweenJS.
					 *  Called internally by CSS property setters when a config.time
					 *  is provided. Can also be called directly for manual tweening.
					 *  Delegates to BetweenJS.create() with the options object:
					 *      { target: node, to: { [prop]: endValue },
					 *        time, ease, delay, initposition, stopOnComplete, cuepoints,
					 *        onStart, onUpdate, onComplete }
					 *  Fires a 'change' event on the dispatcher when complete.
					 *  Falls back to direct style assignment if BetweenJS unavailable.
					 *  @param   {Node}   node     DOM element
					 *  @param   {string} prop     CSS property name
					 *  @param   {*}      endValue Target value
					 *  @param   {Object} cfg      Optional animation config
					 *  @param   {Object} receiver Proxy receiver (for chaining)
					 *  @param   {Object} disp     EventDispatcher (for change event) */
					tween: function (node, prop, endValue, cfg, receiver, disp) {
						if (typeof BetweenJS === 'undefined' || !BetweenJS.create) {
							node.style[prop] = endValue;
							return receiver;
						}
						var opts = {
							target: node,
							to: {},
							time: cfg.time || 400,
							ease: cfg.ease || 'easeOutQuad'
						};
						opts.to[prop] = endValue;
						if (cfg.delay) opts.delay = cfg.delay;
						if (cfg.cuepoints) opts.cuepoints = cfg.cuepoints;
						if (cfg.initposition !== undefined) opts.initposition = cfg.initposition;
						if (cfg.stopOnComplete !== undefined) opts.stopOnComplete = cfg.stopOnComplete;
						['onStart', 'onUpdate', 'onComplete'].forEach(function (k) {
							if (cfg[k]) opts[k] = cfg[k];
						});
						var t = BetweenJS.create(opts);
						t.bind('complete', function () { if (disp) disp.fire('change'); });
						t.start();
						return receiver;
					}
				},
				constructor: DOMNodeProxy = function DOMNodeProxy(node) {
					/** @type {Node} Underlying DOM element */
					this.__target__ = node;
					/** @type {EventDispatcher} Proxy dispatcher wired into Global relay chain.
					 *  Created by Global.generateDomProxyAlongTarget which generates a
					 *  dispatcher pair (model + rider) connected through the Global
					 *  _dispatchers/_proxies tree. Events dispatched here propagate
					 *  to parent dispatchers and vice versa. */
					var disp = Global.generateDomProxyAlongTarget(node);
					this.__dispatcher__ = disp;
					/** Observe DOM mutations and fire 'change' event on dispatcher.
					 *  Provides automatic two-way sync: external DOM changes notify
					 *  declarative directive listeners (data-text, data-bind, etc.). */
					if (typeof MutationObserver !== 'undefined') {
						var obs = new MutationObserver(function () { disp.fire('change'); });
						obs.observe(node, { attributes: true, childList: true, subtree: true, characterData: true });
						this.__observer__ = obs;
					}
					// If native Proxy is unavailable, return the raw DOM node as-is.
					// This keeps the API safe to call in environments without Proxy support
					// but the returned object will be a plain Element, not a DOMNodeProxy.
					if (typeof window.Proxy === 'undefined') return node;
					var handler = makeHandler(node, disp);
					return new window.Proxy(this, handler);
				}
			};
		});

		/** Global `window.prox` alias for DOMNodeProxy.
		 *  Provides concise access to find, wrap, unwrap, and hookup:
		 *      prox.find('#app'), prox.hookup('#app'),
		 *      prox.wrap(el), prox.unwrap(p)
		 *  Does NOT conflict with jQuery's `$` alias — both can coexist. */
		window.prox = DOMNodeProxy;
				...
				if (tempreg.test(temppath) && hh.getLocaleReload()) {
					...
					while (l--) {
						...
						if (!!child.regexp) {
							...
							if (child.id === '' && remainpath !== '') continue;
		...
		/** Module-level cache for fetched API data under the 'session' dataCache strategy. */
		var _sessionDataCache = {};

		/** Detect a CONFIG member (a step described declaratively, as opposed to a
		 *  routes-module object whose values are route functions). An object counts
		 *  as config when it carries '@'-handlers, plain data props (`urljade`, …),
		 *  or config-shaped children. A bare `{}` is an empty container. */
		var isConfigObject = function isConfigObject(o) {
			var k, v, n = 0;
			for (k in o) {
				n++;
				...
				if (typeof v === 'object') {
					if (v.constructor === Object && isConfigObject(v)) return true;
					continue;
				}
				return true;
			}
		...
		/** Strict section-marker test used ONLY to disambiguate the top-level
		 *  `'/'` pattern: a single section config (home) carries `@`-handlers or
		 *  direct data props, while the routes module is a collection of such
		 *  members. Child shapes are deliberately ignored here. */
		var hasSectionMarkers = function hasSectionMarkers(o) {
			var k, v;
			for (k in o) {
				if (k.indexOf('@') == 0) return true;
				v = o[k];
		...
		var Response = Type.define({
			...
			constructor: Response = function Response(id, pattern, commandOpen, commandClose) {
				...
				var focus = function (e) {
					if (e.type == 'focusIn') {
						...
						// Drop the per-open data memo (res.userData.data) on leave, so the
						// next open of this step refetches its urljson data source.
						delete res.userData.data;
					}
				};
				...
				Response.base.apply(this, [
					...
					commandOpen || new Command(res, function resCommandOpen() {
						...
						// Structural rendering defaults — derived from the route tree,
						// never clobbering explicit authoring: a landing step (id '')
						// presents its parent section's identity as `parameters.response`,
						// and inherits the section's viewport when it declares none of its
						// own. Explicit userData/parameters/urljade always win.
						res.userData = res.userData || {};
						res.userData.parameters = res.userData.parameters || {};
						if (res.userData.parameters.response === undefined) {
							res.userData.parameters.response = (res.id === '') ? res.parentStep : res;
						}
						if (res.id === '' && !res.userData.urljade && !!res.parentStep &&
							!!res.parentStep.userData && !!res.parentStep.userData.urljade) {
							res.userData.urljade = res.parentStep.userData.urljade;
						}
			...
			/** Load the route's declared data source (res.userData.urljson) and merge it
			 *  into the Jade template locals (params). Called automatically by render()
			 *  before the template is loaded — routes never call fetch() themselves.
			 *
			 *  Memoization is scoped per-route via res.userData.dataCache:
			 *    unset / 'open'  (default) fetch once per step-open; both template
			 *                    passes of the same open share a single request, and
			 *                    the memo is dropped when the step loses focus
			 *    'session'       cache at module level, keyed by endpoint URL
			 *    false           always fetch fresh on every render()
			 *
			 *  A `lang` query param is appended smartly — only when the endpoint does
			 *  not already carry its own — so one bilingual API can serve the current
			 *  locale. Same-origin endpoints only; no CORS handling.
			 *
			 *  @param {string}   urljson  Data source endpoint (same-origin backend API)
			 *  @param {Object}   params   Jade/i18n parameters (template locals)
			 *  @param {Function} [cb]     Called with merged params once data is in
			 *  @return {Object} params    Returned for backward-compatible sync callers */
			fetch: function (urljson, params, cb) {
				var res = this, request, url = urljson;
				...
				var strategy = (res.userData && res.userData.dataCache !== undefined) ? res.userData.dataCache : 'open';
				var memo = (strategy === 'session') ? _sessionDataCache[url]
					: (!strategy || !res.userData) ? undefined
					: res.userData.data;

				if (memo !== undefined) {
					Object.assign(params, memo);
					if (!!cb) cb(params);
					return params;
				}

				// Smart lang: append ?lang=<current locale> unless the endpoint sets its own
				var lang = (typeof i18next !== 'undefined' && !!i18next.language) ? i18next.language
					: document.documentElement.getAttribute('lang');
				if (!!lang && !/[?&]lang=/.test(url)) {
					url += (url.indexOf('?') != -1 ? '&' : '?') + 'lang=' + encodeURIComponent(lang);
				}

				var gotData = function (data) {
					if (!!data && typeof data === 'object' && !(data instanceof Array)) {
						if (strategy === 'session') _sessionDataCache[url] = data;
						else if (!!strategy && !!res.userData) res.userData.data = data;
						Object.assign(params, data);
					} else {
						Logger.warn('StraExpress fetch: expected a JSON object from ' + url);
					}
					if (!!cb) cb(params);
				};

				try {
					request = new Request().load(true, url, function (jxhr, r) {
						var data = {};
						try { data = JSON.parse(r) || {}; }
						catch (e) {
							Logger.error('StraExpress fetch: invalid JSON from ' + url);
							data = {};
						}
						gotData(data);
					}, null, function (xhr, u) {
						Logger.error('StraExpress fetch failed: ' + u);
						if (!!cb) cb(params);
					});
				} catch (e) {
					Logger.error('StraExpress fetch error: ' + url);
					if (!!cb) cb(params);
				}
			 ...
			 *  When res.userData.urljson is set, the data source is fetched FIRST and
			 *  merged into params so fetched data is present when jade.render runs.
			...
			render: function (url, params, callback) {
				...
				if (!!callback) { // switch to ASYNC MODE
					...
					// Load the route's data source first (when declared), then the template.
					var loadTemplate = function (p) {
						var hash = '?' + res.generateHash();
						var request, t;
						try {
							request = new Request().load(true, url + hash, function (jxhr, r) {
								t = request.response;
								res.packResponse(t, url, p, function(template){
									res.template = template ;
									callback.apply(res, [].concat(args));
								});
							}, null, function (r, url) {
								Logger.error('Template render XHR failed: ' + url);
								res.packResponse('', url, p, function(template){
									res.template = template;
									callback.apply(res, [].concat(args));
								});
							});
					...
					if (res.userData && res.userData.urljson) {
						res.fetch(res.userData.urljson, params, loadTemplate);
					} else {
						loadTemplate(params);
					}
				} else {
		...
		/* BackgroundTaskQueue — offloads heavy init to post-boot frame chain */

		/** @class BackgroundTaskQueue
		 *  A priority task queue that drains tasks across animation frames,
		 *  keeping the main thread free for user interaction. Tasks are
		 *  organized into three tiers:
		 *
		 *  - **Tier 0** (critical): Runs synchronously at `start()`. Use for
		 *    the single most important pre-warm (e.g. one shader compile).
		 *    Tasks receive no `done` callback — they must be sync.
		 *
		 *  - **Tier 1** (background): Runs one task per animation frame via
		 *    `BetweenJS.timeout(0, ...)`. Tasks receive a `done()` callback
		 *    they must call when finished (supports async work).
		 *
		 *  - **Tier 2** (on-demand): Does **not** auto-drain. Call
		 *    `flush(2)` explicitly (e.g. on user interaction or idle).
		 *
		 *  **Requires BetweenJS** for frame scheduling. If BetweenJS is not
		 *  loaded, a warning is logged and tier 1 falls back to
		 *  `setTimeout(fn, 0)` — functional but not frame-aligned.
		 *
		 *  @example
		 *  var q = new BackgroundTaskQueue();
		 *  q.add(function() { precompileShader(heroConfig); }, 0);  // sync, runs at start()
		 *  q.add(function(done) { compileShader(bg, done); }, 1);   // async, one per frame
		 *  q.add(function(done) { loadPointCloud(done); }, 1);      // async, next frame
		 *  q.start();  // tier 0 runs now, tier 1 chains via rAF
		 *
		 *  @fires BackgroundTaskQueue#taskComplete  After each task finishes
		 *  @fires BackgroundTaskQueue#complete       When all tiers are drained
		 *  @constructor */
		var BackgroundTaskQueue = Type.define({
			pkg: '::BackgroundTaskQueue',
			domain: Type.appdomain,

			constructor: function BackgroundTaskQueue() {
				var q = this;
				q.tiers = [[], [], []];
				q.running = false;
				q.paused = false;
				...
				/** Warn if BetweenJS is missing */
				if (typeof BetweenJS === 'undefined' && typeof BJS === 'undefined') {
					Logger.warn('BackgroundTaskQueue: BetweenJS not found — tier 1 will use setTimeout fallback (not frame-aligned)');
				}
			...
			statics: {
				/** Check if BetweenJS is available
				 *  @return {boolean} */
				hasBJS: function hasBJS() {
					return typeof BetweenJS !== 'undefined' || typeof BJS !== 'undefined';
				},

				/** Get the BJS reference (either BetweenJS or BJS global)
				 *  @return {Object|null} */
				getBJS: function getBJS() {
					return (typeof BetweenJS !== 'undefined') ? BetweenJS : (typeof BJS !== 'undefined') ? BJS : null;
				}
			},

			/** Add a task to a tier.
			 *  @param {Function} task     Tier 0: `function()`. Tiers 1-2: `function(done)` where `done()` signals completion.
			 *  @param {number}   [tier=1] Priority tier (0, 1, or 2)
			 *  @return {BackgroundTaskQueue} this */
			add: function add(task, tier) {
				tier = tier || 1;
				if (tier < 0 || tier > 2) {
					Logger.warn('BackgroundTaskQueue: invalid tier ' + tier + ', clamping to 1');
					tier = 1;
				}
				this.tiers[tier].push(task);
				this.taskCount++;
				return this;
			},

			/** Begin processing. Tier 0 runs synchronously now. Tier 1 chains
			 *  via rAF (or setTimeout fallback). Tier 2 does not auto-drain.
			 *  @return {BackgroundTaskQueue} this */
			...
			/** Pause tier 1 processing. Tier 0 is unaffected (sync).
			 *  @return {BackgroundTaskQueue} this */
			...
			/** Resume tier 1 processing after pause.
			 *  @return {BackgroundTaskQueue} this */
			resume: function resume() {
				if (!this.paused) return this;
				this.paused = false;
				if (!this._draining && this.tiers[1].length > 0) {
					this._drainTier(1);
				}
				return this;
			},

			/** Cancel tasks. If tier is given, clear only that tier.
			 *  Otherwise clear all tiers.
			 *  @param {number} [tier]  Tier to clear. Omit = all.
			 *  @return {BackgroundTaskQueue} this */
			cancel: function cancel(tier) {
				if (typeof tier === 'number') {
					this.taskCount -= this.tiers[tier].length;
					this.tiers[tier] = [];
				} else {
					this.taskCount = 0;
					this.tiers = [[], [], []];
			...
			/** Manually drain a specific tier. For tier 2, call this on
			 *  idle / user interaction. For tiers 0-1, called by `start()`.
			 *  @param {number} tier
			 *  @return {BackgroundTaskQueue} this */
			flush: function flush(tier) {
				if (tier === 0) {
					this._drainTier(0);
				} else if (tier === 1) {
					if (!this._draining) this._drainTier(1);
				} else if (tier === 2) {
					this._drainTier(2);
				}
				return this;
			...
			/** Number of tasks remaining across all tiers.
			 *  @return {number} */
			pending: function pending() {
				return this.tiers[0].length + this.tiers[1].length + this.tiers[2].length;
			},

			/** @private Drain tier 0 synchronously, tiers 1-2 one-per-frame. */
			_drainTier: function _drainTier(tier) {
				var q = this;
				var tasks = q.tiers[tier];

				if (tasks.length === 0) {
					if (tier === 1) q._checkComplete();
					return;
				}
				...
				if (tier === 0) {
					/** Tier 0: synchronous, no done callback */
					try {
						task();
					} catch (e) {
						Logger.error('BackgroundTaskQueue: tier 0 task threw — ' + e.message);
					}
					/** Recurse for remaining tier 0 tasks */
					if (tasks.length > 0) q._drainTier(0);
					else q._checkComplete();
				} else {
					/** Tier 1-2: async, task calls done() when finished */
					q._draining = true;
					...
					var done = function () {
						...
						/** Schedule next task on the following frame */
						if (tasks.length > 0 && q.running && !q.paused) {
							q._scheduleNext(tier);
						} else {
							q._checkComplete();
						}
					};
				...
				try {
					...
					Logger.error('BackgroundTaskQueue: tier ' + tier + ' task threw — ' + e.message);
					done();
				}
				}
			},

			/** @private Schedule the next task via BetweenJS or setTimeout fallback. */
			_scheduleNext: function _scheduleNext(tier) {
				var q = this;
				var bjs = BackgroundTaskQueue.getBJS();

				if (bjs) {
					bjs.timeout(0, function () {
						q._drainTier(tier);
					}).play();
				} else {
					setTimeout(function () {
			...
			/** @private Check if all tiers are drained, fire complete event. */
			...
			/** Destroy the queue and release references. */
			destroy: function destroy() {
				this.cancel();
				this.running = false;
				this.paused = false;
		...
		var Express = Type.define({
			 ...
			 *  are recursively registered. A non-function handler that is a CONFIG
			 *  member (carries `@`-handlers, data props, or config children) is
			 *  converted by `handlerFromConfig` into a pass-through handler. Named
			 *  param patterns (`/:id`) are auto-detected in the Response constructor.
			...
			get: function get(pattern, handler, parent) {
				...
				if (handler.constructor !== Function) {
					...
					// CONFIG MEMBER vs ROUTES MODULE: at pattern '/' an object is the
					// routes-module itself (or a single home section — strict markers
					// tell them apart); at any other pattern a non-function object is
					// a declarative config member (data props → userData, '@'-props →
					// handlers, object values → child routes) and is converted to a
					// pass-through handler.
					if (handler && typeof handler === 'object' && !(handler instanceof Array) && !(handler instanceof RegExp) &&
						(pattern == '/' ? hasSectionMarkers(handler) : isConfigObject(handler))) {
						handler = this.handlerFromConfig(handler);
					} else {
			...
			/** Convert a declarative CONFIG object into a route handler function.
			 *  The synthesized handler is a pass-through; its lifecycle behavior comes
			 *  from its `@`-props (attached by `enableResponse`'s walk). Pure containers
			 *  complete immediately, rendering members defer `ready()` to their
			 *  `@toggle`. All source props are copied onto the handler so the walk
			 *  below can hoist/handle them.
			 *  @param {Object} cfg  A config member object
			 *  @return {Function} The synthesized handler */
			handlerFromConfig: function handlerFromConfig(cfg) {
				var renders = false, s;
				for (s in cfg) {
					if (s.indexOf('@') > -1 && (s == '@toggle' || s == '@open' || s == '@toggleIn')) renders = true;
				}
				var fn = renders
					? function (req, res) { return res; }
					: function (req, res) { return res.ready(); };
				for (s in cfg) fn[s] = cfg[s];
				return fn;
			},
			...
			enableResponse: function enableResponse(cond, res, parent) {
				...
				if (cond) {
					...
					for (var s in handler) {
						...
						var hv = handler[s];
						if (s.indexOf('@') == 0) this.attachHandler(true, s, hv, res);
						else if (s == 'index') this.get('', hv, res);
						else if (s == 'userData') res.userData = Object.assign(res.userData || {}, hv);
						else if (typeof hv === 'function') this.get(s, hv, res);
						else if (hv && typeof hv === 'object') this.get(s, hv, res);
						else res.userData[s] = hv; // plain data prop (urljade, …) → hoist to userData
					}
			 ...
			 *
			 *  This same mapping is replicated in DOMNodeProxy's on/off/once/trigger
			 *  methods (via resolveEvent at the module level). The two implementations
			 *  are independent — DOMNodeProxy does not depend on Express — but produce
			 *  equivalent bindings through the Global event relay chain. When a
			 *  DOMNodeProxy-wrapped element lives inside a Step's DOM subtree, events
			 *  dispatched here propagate to the proxy's listeners automatically.
			 *
