	...
	return Pkg.write('org.libspark.straw', function () {
		...
		var Global = Type.define({
			...
			statics: {
				events: {},
				all: [],
				_domProxyMap: typeof WeakMap !== 'undefined' ? new WeakMap() : null, // 2.0: O(1) DOM-target → model lookup
				IEvent: IEvent,
				evtypeindex: 0,
				...
				retrieveModelProxy: function (p) {
					// 2.0: O(1) WeakMap lookup instead of O(n) array scan
				...
				if (!!callback) { // switch to ASYNC MODE
					...
					try {
						...
						/* 
							...
							jade.renderFile(url, params, function(err, template){
								res.template = $(template) ;
								trace(template)
								callback.apply(res, [].concat(args));
							})
